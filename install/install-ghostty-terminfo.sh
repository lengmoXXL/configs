#!/bin/bash
# 在服务器端安装 Ghostty 的 terminfo 条目（xterm-ghostty），
# 解决从 Ghostty SSH 登录后 Oh My Bash/readline/tput 等终端能力查询失败的问题。
# 条目内嵌在本脚本中（取自 macOS Ghostty.app 的 terminfo，infocmp -x 导出），无需联网。
# 如需更新内嵌条目：在装有新版 Ghostty 的 macOS 上执行 `infocmp -x xterm-ghostty`，
# 用输出整体替换下方 TERMINFO_EOF 之前的内容。
...
if ! command -v tic &>/dev/null; then
    echo "错误: 缺少 tic 命令，请先安装 ncurses（Debian/Ubuntu: apt install ncurses-bin；CentOS: yum install ncurses）"
    exit 1
fi
...
# tic 默认安装到 ~/.terminfo，无需 root
tic -x "$TERMINFO_SRC"

if infocmp xterm-ghostty &>/dev/null; then
    echo "xterm-ghostty terminfo 已安装到 ~/.terminfo"
else
    echo "错误: 安装后仍无法查询 xterm-ghostty terminfo"
    exit 1
fi
